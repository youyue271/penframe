...
go mod tidy
go test ./...
go run ./cmd/opo -tools /path/to/tools.yaml -workflow /path/to/workflow.yaml
go run ./cmd/opo -tools /path/to/tools.yaml -workflow /path/to/workflow.yaml -target https://target.example:3000
...
- `local` 默认跟随当前环境的 `$SHELL`，也可在节点上通过 `shell` 字段覆盖
- `local` 支持节点级 `env` 覆盖，适合 live workflow 精确控制 `HTTP_PROXY` / `HTTPS_PROXY` / `NO_PROXY`
...
- 节点 `metadata` 还会记录 `shell_path` 与 `env_keys`，便于排查 live 节点的环境差异
...
- 可通过 `vars` 覆盖 live workflow 的工具链开关与参数（如 `run_http_entry_probe`、`run_http_signin_probe`、`run_http_install_probe`、`run_http_console_api_probe`、`run_nmap_quick`、`run_nuclei_web`、`http_probe_timeout_seconds`、`curl_extra_args`、`nmap_quick_args`、`fscan_args`、`nuclei_args`）
...
5. 在现有 SSE 基础上补节点级日志视图与输出文件 tail
